...
5. 각 dataset 은 train split 과 test split 을 각각 단일 physical payload 파일 하나로 저장한다.
...
10. `data_prep` 가 새로 생성하는 공식 산출물은 dataset 당 `train.npy`, `test.npy`, `manifest.json` 뿐이다.
...
3. `manifest.json` 은 dataset 단위의 생성 metadata 와 runtime view 해석 규칙을 담는 소형 요약 파일이다.
...
정적 이미지 데이터 `mnist`, `cifar-10` 은 PLIF식 direct input으로 처리한다. 같은 image frame을 `data_prep` 저장 단계에서 4 timestep으로 반복하여 physical storage 자체를 `(T,C,H,W)` 로 만든다. 모델 코드와 runtime view는 반복 frame을 새로 만들지 않는다.

| dataset | stored_view_name | stored_shape | logical PSD shape | axis metadata 요약 |
| --- | --- | --- | --- | --- |
| `mnist` | `model_input` | `(4, 1, 28, 28)` | `(784, 4)` | `T=4`, row axis는 channel-height-width flatten, time axis는 static repeat timestep |
| `cifar-10` | `model_input` | `(4, 3, 32, 32)` | `(3072, 4)` | `T=4`, row axis는 channel-height-width flatten, time axis는 static repeat timestep |
...
`model_input`, `psd_input`, `image_psd_view` 는 모두 `(T,C,H,W)` 를 반환한다. CNN 계열 모델은 dataloader batch인 `(B,T,C,H,W)` 를 그대로 받는다. Dense 계열 모델만 `canonicalize_model_input_batch` 단계에서 `(B,T,C*H*W)` 로 변환한다. PSD 분석은 `tensor_to_channel_major_maps` 단계에서만 `(B,T,C,H,W) -> (B,C*H*W,T)` 로 flatten 한다.
...
1. `dataset_name`
2. `files.train = "train.npy"`
3. `files.test = "test.npy"`
...
2. `manifest.json` 은 `train.npy` 와 `test.npy` 검증이 모두 끝난 뒤에만 기록한다.
3. `force_overwrite` 가 false 인 상태에서 기존 payload 가 있으면 덮어쓰지 않는다.

## 9. loader 규칙

runtime loader 는 `train.npy` 또는 `test.npy` 를 `np.load(path, mmap_mode="r")` 로 연다. loader 는 split 전체를 RAM 으로 materialize 하면 안 된다.
...
3. `manifest.json` 이 가리키는 `train.npy` 와 `test.npy` 만 사용한다.
...
8. 여러 모델과 여러 seed 는 같은 `train.npy`, `test.npy` 를 공통 참조해야 하며 per-run 복사본을 만들면 안 된다.
9. supervised training 과 inference 는 항상 `manifest.json` 의 `training_view_name` 과 model input axis metadata 를 사용해야 한다.
...
7. `train.npy` 또는 `test.npy` 경로를 timestamp 나 실험 시나리오 의존 이름으로 만들어 split 내부 고정 순서 재현성을 해치면 안 된다.
...
```text
static_repeat_T = 4
mnist stored/model_input shape = (4, 1, 28, 28)
cifar-10 stored/model_input shape = (4, 3, 32, 32)
sequence_length = 4
input_dim = C*H*W
cnn_input_shape = [4, C, H, W]
```
