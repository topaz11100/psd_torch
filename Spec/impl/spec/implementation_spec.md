...
요구사항:
개편 project 는 세 개의 official PSD-stage entrypoint 를 갖는다.
...
- `src/psd_analysis.py` 는 checkpoint-analysis-only program 으로 존재한다.
- `src/plotting.py` 는 visualization-only program 으로 존재한다.
- 세 entrypoint 는 서로를 암묵적으로 호출하지 않는다.
...
- family spectral trace, dispersion trace, pair-distance trace, filter trend, accuracy/loss join 은 checkpoint metadata 와 analysis output 에서 계산한다.
...
- `train_same_label_label_<c>`, `test_same_label_label_<c>`, `train_balanced_global`, `test_balanced_global`, `train_distribution_global`, `test_distribution_global` probe scope 를 처리한다.
