...
- "가변 가지 수 $s$ " 및 "수상돌기 LTI 해석 + 상관 정규화" 는 별도 문서 **variable_dendritic_multiplicity.md** 에서 공통 이론으로 정리한다.
...
$$
 o_{n}^{t+1,l}=H\!\left(\tilde{u}_{n}^{t+1,l}-u_{\mathrm{th},n}\right)
$$

이다. 이후 reset mode 에 따라 세포체 상태만 갱신한다.

$$
u_{n}^{t+1,l}=
\begin{cases}
\tilde{u}_{n}^{t+1,l}-u_{\mathrm{th},n}o_{n}^{t+1,l}, & \text{soft reset}\\
\tilde{u}_{n}^{t+1,l}(1-o_{n}^{t+1,l}), & \text{hard reset}\\
\tilde{u}_{n}^{t+1,l}, & \text{no reset}
\end{cases}
$$

이 reset은 세포체 상태 \(u\) 에만 적용된다. 가지 상태 \(i_{n,d}\), branch mask \(M\), branch count \(s\) 는 reset되지 않는다. `v_th: ["fixed", x]`는 고정 \(u_{\mathrm{th},n}=x\) 를 만들고, `v_th: ["train", x]`는 per-soma positive threshold parameter 를 만든다.
...
- 필요하면 $\alpha$ 에 상한을 두어 $\alpha\to 1$ 근방 민감도를 줄인다( variable_dendritic_multiplicity.md 권장).
...
- **기본 초기화 정책:** `s_init` 을 별도로 지정하지 않으면 **S_max 근처에서 시작** 한다.
  - 구현은 sigmoid 포화를 피하기 위해 정규화 비율을 $[\epsilon,1-\epsilon]$ 로 clamp 하므로, 실제 초기 $s$ 는
    $S_{\max}-\epsilon(S_{\max}-S_{\min})$ 처럼 S_max보다 약간 작은 값이 된다(자세한 근거는 variable_dendritic_multiplicity.md §2.3).
...
- $\mathcal{L}_{\text{ortho}}$ : 가지 시간필터($\alpha$) 중복 억제
  ...
  - 닫힌형은 variable_dendritic_multiplicity.md 참조
...
- reset: `soft`, `hard`, `none` 중 선택하며 세포체 상태에만 적용
- $s$: **뉴런 단위** $s_{n}^{l}$ 를 사용
...
- 세포체 집계: $1/s_{n}^{l}$ 적용
- 규제항: $\lambda_{\text{ortho}},\lambda_s$ (variable_dendritic_multiplicity.md)
- 학습: surrogate 종류, BPTT 길이, gradient clipping

## Direct discrete interpretation update

`my_DH_SNN`은 branch별 EMA pole을 직접 학습하는 이산시간 모델로 해석한다.

\[
i_{n,d}[t]=\alpha_{n,d}i_{n,d}[t-1]+(1-\alpha_{n,d})I_{n,d}[t]
\]

\[
\tilde U_n[t]=\beta_nU_n[t-1]+(1-\beta_n)H_n[t],\quad S_n[t]=H(\tilde U_n[t]-V_{th,n})
\]
\[
U_n[t]=\operatorname{soma\_reset}(\tilde U_n[t], S_n[t], V_{th,n})
\]

따라서 `alpha`는 `branch_pole_radius`, `beta`는 `soma_pole_radius` 와 동의어다. 이 값들은 연속시간 시정수에서 유도한 값이 아니라 sample-domain decay factor 로 기록한다.
