# my_R_DH_SNN_neuron — Reverse DH-SNN + soma-dense 입력(KM) + dendrite-broadcast + soma pole/reset + 가변 가지 수

## 0. 목적

Reverse DH-SNN 은 다중 수상돌기 필터와 세포체 집계를 분리하되, 레이어 연결은 일반 dense 처럼 두고 뉴런 내부 가지에는 동일한 입력을 broadcast 하는 구조를 뜻한다.

본 명세의 핵심 변경점은 세포체 입력은 dense projection으로 만들고, 가지 축은 동일한 soma-drive를 broadcast해서 variable-branch filter bank로 해석한다는 점이다. 현재 구현의 세포체는 `beta_soma`를 갖는 discrete EMA soma이며, reset/threshold 정책은 vanilla layer와 같은 soma-local contract를 따른다.
...
- 수상돌기 전류는 가지별 EMA 로 유지하고, 세포체는 `beta_soma` EMA로 집계 입력을 누적한 뒤 선택된 soma reset policy로 스파이크를 생성한다.

가변 가지 수 $s$ , 소프트 마스크, 상관 정규화의 공통 이론은 `variable_dendritic_multiplicity.md` 를 따른다.
...
$1/D$ 가 아니라 $1/s$ 를 사용하는 이유와 소프트 마스크 해석은 `variable_dendritic_multiplicity.md` 를 따른다.

### 2.4 세포체 EMA 누적 및 configurable soma reset

현재 구현의 세포체는 집계 입력을 EMA로 누적한다.
...
$$
o[t] = \Theta\!\left(u_{pre}[t]-u_{\mathrm{th}}\right)
$$

스파이크 이후 세포체 상태는 `reset=soft|hard|none`에 따라 soft subtraction, hard zeroing, no reset 중 하나를 따른다. 이 reset은 세포체 상태에만 적용되고 가지 EMA 상태에는 적용되지 않는다. 따라서 세포체의 역할은 다음 두 가지로 요약된다.
...
### 3.4 세포체 입력과 soma pole/reset 업데이트
...
로 계산한다. 이후 `reset=soft|hard|none`에 따라 세포체 상태만 갱신한다.
...
- 비적용 대상: 구조 파라미터 $s$, 가지 상태전이계수 $\alpha$, 세포체 상태전이계수 $\beta$, trainable soma threshold

R-DH 구현은 세포체 pole $\beta$ 를 갖는다. 따라서 timing/filter 통계에는 branch $\alpha$ 와 soma $\beta$ 가 모두 기록된다.
...
- 세포체는 `beta_soma` EMA + configurable soma reset이며, reset은 dendritic branch state에는 적용되지 않는다.
- timing factor 는 가지별 $\alpha$ 만 기록한다.
- 세포체는 집계 전류 메모리와 스파이크 생성기의 역할만 맡는다.

## Direct discrete interpretation update

`my_R_DH_SNN`은 branch별 EMA pole과 branch mixing weight를 직접 학습하는 이산시간 모델로 해석한다.

\[
i_{n,d}[t]=\alpha_{n,d}i_{n,d}[t-1]+(1-\alpha_{n,d})I_n[t]
\]

\[
H_n[t]=\frac{1}{s_n}\sum_d M_{n,d}(s_n)w_{n,d}i_{n,d}[t]
\]

`alpha`는 `branch_pole_radius`, `beta`는 `soma_pole_radius` 로도 저장된다. PSD/filter-property analysis 에서는 EMA branch 전달함수 \((1-\alpha)/(1-\alpha z^{-1})\) 를 사용한다.
