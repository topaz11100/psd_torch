...
# 기능: training 이 저장한 .pt checkpoint 를 불러와 prepared probe set 에 대한 MLP output map 2-D FFT power matrix CSV 를 생성한다.
...
# 산출물: `<OUTPUT_ROOT>/<case_id>` 아래 analysis_2d_fft category CSV 를 생성한다.
# 주의: 이 launcher 는 figure rendering 을 실행하지 않는다.
# 로그: `<LOG_ROOT>/2d_fft_analysis/<RUN_STAMP>` 아래 job별 log, temporary checkpoint batch directory, pid.tsv 를 생성한다.
...
# 값 주는법: `PROJECT_ROOT=/absolute/path/to/psd bash bash/2d_fft_analysis.sh` 처럼 준다.
PROJECT_ROOT="${PROJECT_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"

# 인수: PYTHON_BIN
# 기능: child job 을 실행할 Python executable 을 지정한다.
# 줄 수 있는 값: `python3`, `python`, conda 환경의 `/path/to/python` 같은 실행 가능 파일.
# 값의 의미: 지정한 Python 으로 `src.2d_fft_analysis` module 을 실행한다.
# 값 주는법: `PYTHON_BIN=/home/user/miniconda3/envs/snn/bin/python bash bash/2d_fft_analysis.sh` 처럼 준다.
...
CHECKPOINT_SET_RAW="/home/yongokhan/바탕화면/new/shd/checkpoints/shd_lif_soft_fixed_128_64_64_temporal_membrane_e50_b256_lr0.005_seed0_reg_l10_l20_sigy_spike_spaceexact_scaledb_centerraw_redmean/checkpoint_epoch_000001.pt /home/yongokhan/바탕화면/new/shd/checkpoints/shd_lif_soft_fixed_128_64_64_temporal_membrane_e50_b256_lr0.005_seed0_reg_l10_l20_sigy_spike_spaceexact_scaledb_centerraw_redmean/checkpoint_epoch_000035.pt"
...
OUTPUT_ROOT="/home/yongokhan/바탕화면/new/shd/2d_fft"

# 인수: LOG_ROOT
# 기능: launcher log 와 pid.tsv 를 저장할 root directory 를 지정한다.
# 줄 수 있는 값: 반드시 제공해야 하는 경로 문자열.
# 값의 의미: 실제 log directory 는 `<LOG_ROOT>/2d_fft_analysis/<RUN_STAMP>` 이다.
...
# 인수: LOW_VRAM
# 기능: 2d_fft_analysis 의 VRAM 절약 경로 사용 여부를 지정한다.
# 줄 수 있는 값: `0`, `1`. 기본값은 `0` 이다.
# 값의 의미: `1` 이면 Python process 에 `--low_vram 1` 을 전달하여 VRAM 사용량을 줄이는 분석 경로를 사용한다.
# 값 주는법: `LOW_VRAM=1 bash bash/2d_fft_analysis.sh` 처럼 준다.
LOW_VRAM="${LOW_VRAM:-0}"

LOG_DIR="${LOG_ROOT}/2d_fft_analysis/${RUN_STAMP}"
...
if [[ ${#GPU_INDEX_SET[@]} -lt 1 || -z "${GPU_INDEX_SET[0]:-}" ]]; then
    echo '[2d_fft_analysis.sh] GPU_INDEX_SET must contain at least one GPU index.' >&2
    exit 1
fi
...
_collect_checkpoint_files() {
    ...
    if [[ -f "${entry}" ]]; then
        if [[ "${entry}" != *.pt ]]; then
            echo "[2d_fft_analysis.sh] checkpoint file must end with .pt: ${entry}" >&2
            exit 1
        fi
        printf '%s
' "$(realpath "${entry}")"
    ...
    if [[ ! -d "${entry}" ]]; then
        echo "[2d_fft_analysis.sh] checkpoint input does not exist or is not a file/directory: ${entry}" >&2
        exit 1
    fi
    local subdir
    subdir="$(find "${entry}" -mindepth 1 -maxdepth 1 -type d -print -quit)"
    if [[ -n "${subdir}" ]]; then
        echo "[2d_fft_analysis.sh] strict checkpoint directory may not contain subdirectories: ${subdir}" >&2
        exit 1
    fi
    local non_pt
    non_pt="$(find -L "${entry}" -mindepth 1 -maxdepth 1 -type f ! -name '*.pt' -print -quit)"
    if [[ -n "${non_pt}" ]]; then
        echo "[2d_fft_analysis.sh] strict checkpoint directory may contain .pt files only: ${non_pt}" >&2
        exit 1
    fi
    mapfile -t found_files < <(find -L "${entry}" -mindepth 1 -maxdepth 1 -type f -name '*.pt' -print | sort)
    if [[ ${#found_files[@]} -lt 1 ]]; then
        echo "[2d_fft_analysis.sh] checkpoint directory contains no .pt files: ${entry}" >&2
        exit 1
    fi
    local file
...
if [[ ${#checkpoint_files[@]} -lt 1 ]]; then
    echo '[2d_fft_analysis.sh] no checkpoint files were discovered.' >&2
    exit 1
fi
...
if [[ "${CHECKPOINTS_PER_JOB}" == "all" ]]; then
    ...
    echo '[2d_fft_analysis.sh] CHECKPOINTS_PER_JOB must be a positive integer or all.' >&2
    exit 1
fi
...
for ((start=0; start<${#checkpoint_files[@]}; start+=batch_size)); do
    ...
    log_file="${LOG_DIR}/2d_fft_analysis__${RUN_STAMP}__${slot_id}.log"
    cmd=("${PYTHON_BIN}" -m src.2d_fft_analysis
...
' "${RUN_STAMP}" "2d_fft_analysis" "${case_id}" "${slot_id}" "$!" "${log_file}" "${command_text}" >> "${PID_FILE}"
    launch_count=$((launch_count + 1))
done

printf '[2d_fft_analysis.sh] launched %d job(s); pid file: %s
