...
launch_config() {
    ...
    CONFIG_PATH="$CONFIG_PATH" nohup bash -c 'python src/2d_fft_analysis.py --config "$CONFIG_PATH"' > "$LOG_PATH" 2>&1 &
    PID="$!"
    LAST_PID="$PID"
    echo "STAGE=${STAGE}"
