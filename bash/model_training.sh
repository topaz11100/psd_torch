...
CONFIG_GROUPS=(CONFIG_GROUP_0 CONFIG_GROUP_1 CONFIG_GROUP_2 CONFIG_GROUP_3)
...
launch_config() {
    ...
    nohup bash -c 'python src/model_training.py --config "$CONFIG_PATH"' > "$LOG_PATH" 2>&1 &
    PID="$!"
    LAST_PID="$PID"
    echo "STAGE=${STAGE}"
