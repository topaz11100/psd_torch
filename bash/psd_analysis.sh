...
# 기능: training 이 저장한 .pt checkpoint 를 불러와 prepared probe set 에 대한 PSD analysis CSV 를 생성한다.
...
# 분석 범위: psd_analysis 는 train/test 전체 집합을 분석하지 않고, prepared probe set 만 분석한다.
# 산출물: `<OUTPUT_ROOT>/<case_id>` 아래 analysis category별 CSV 를 생성한다.
# 주의: 이 launcher 는 figure rendering 을 실행하지 않는다.
# 로그: `<LOG_ROOT>/psd_analysis/<RUN_STAMP>` 아래 job별 log, temporary checkpoint batch directory, pid.tsv 를 생성한다.
...
# 값 주는법: `PROJECT_ROOT=/absolute/path/to/psd bash bash/psd_analysis.sh` 처럼 준다.
PROJECT_ROOT="${PROJECT_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"

# 인수: PYTHON_BIN
# 기능: child job 을 실행할 Python executable 을 지정한다.
# 줄 수 있는 값: `python3`, `python`, conda 환경의 `/path/to/python` 같은 실행 가능 파일.
# 값의 의미: 지정한 Python 으로 `src.psd_analysis` module 을 실행한다.
# 값 주는법: `PYTHON_BIN=/home/user/miniconda3/envs/snn/bin/python bash bash/psd_analysis.sh` 처럼 준다.
...
# 줄 수 있는 값: 반드시 제공해야 하는 경로 문자열.
# 값의 의미: 실제 log directory 는 `<LOG_ROOT>/psd_analysis/<RUN_STAMP>` 이다.
...
# 값의 의미: probe set 을 몇 sample 단위로 나누어 model forward 와 PSD accumulation 을 할지 정한다.
...
# 값의 의미: 빈 값이면 Python 기본값을 쓰고, 정수를 주면 `--seed` 로 전달하여 probe/data order 관련 seed 를 고정한다.
...
# PSD extractor policy: checkpoint PSD analysis is exact-only.
# userbin analysis and userbin CSV artifacts are intentionally disabled.

# 인수: LOW_VRAM
# 기능: psd_analysis 의 VRAM 절약 경로 사용 여부를 지정한다.
# 줄 수 있는 값: `0`, `1`. 기본값은 `0` 이다.
# 값의 의미: `1` 이면 Python process 에 `--low_vram 1` 을 전달하여 VRAM 사용량을 줄이는 분석 경로를 사용한다.
# 값 주는법: `LOW_VRAM=1 bash bash/psd_analysis.sh` 처럼 준다.
LOW_VRAM="${LOW_VRAM:-0}"

# 인수: ENABLE_PAIRWISE_DEPENDENCY_APPENDIX
# 기능: pairwise dependency appendix CSV 를 추가 생성할지 지정한다.
# 줄 수 있는 값: 참 값 `1`, `true`; 거짓 값은 그 외 값 또는 빈 값이다.
# 값의 의미: 참이면 Python 에 `--enable_pairwise_dependency_appendix` flag 를 붙인다.
# 값 주는법: `ENABLE_PAIRWISE_DEPENDENCY_APPENDIX=1` 또는 `ENABLE_PAIRWISE_DEPENDENCY_APPENDIX=true` 처럼 준다.
ENABLE_PAIRWISE_DEPENDENCY_APPENDIX="${ENABLE_PAIRWISE_DEPENDENCY_APPENDIX:-0}"

# 인수: ANALYSIS_DISTANCE_METRIC
# 기능: psd_analysis 에서 PSD 곡선 개형거리 계산에 사용할 거리 지표를 지정한다.
# 줄 수 있는 값: `centered_l2`, `diff_l2`.
# 값의 의미: `centered_l2` 는 기존 평균 제거 후 L2, `diff_l2` 는 정규화 없는 1차 차분 L2 이다.
# 값 주는법: `ANALYSIS_DISTANCE_METRIC=diff_l2 bash bash/psd_analysis.sh` 처럼 준다.
ANALYSIS_DISTANCE_METRIC="${ANALYSIS_DISTANCE_METRIC:-diff_l2}"

LOG_DIR="${LOG_ROOT}/psd_analysis/${RUN_STAMP}"
...
if [[ ${#GPU_INDEX_SET[@]} -lt 1 || -z "${GPU_INDEX_SET[0]:-}" ]]; then
    echo '[psd_analysis.sh] GPU_INDEX_SET must contain at least one GPU index.' >&2
    exit 1
fi
...
_collect_checkpoint_files() {
    ...
    if [[ -f "${entry}" ]]; then
        if [[ "${entry}" != *.pt ]]; then
            echo "[psd_analysis.sh] checkpoint file must end with .pt: ${entry}" >&2
            exit 1
        fi
        printf '%s\n' "$(realpath "${entry}")"
        return
    ...
    if [[ ! -d "${entry}" ]]; then
        echo "[psd_analysis.sh] checkpoint input does not exist or is not a file/directory: ${entry}" >&2
        exit 1
    fi
    local subdir
    subdir="$(find "${entry}" -mindepth 1 -maxdepth 1 -type d -print -quit)"
    if [[ -n "${subdir}" ]]; then
        echo "[psd_analysis.sh] strict checkpoint directory may not contain subdirectories: ${subdir}" >&2
        exit 1
    fi
    local non_pt
    non_pt="$(find -L "${entry}" -mindepth 1 -maxdepth 1 -type f ! -name '*.pt' -print -quit)"
    if [[ -n "${non_pt}" ]]; then
        echo "[psd_analysis.sh] strict checkpoint directory may contain .pt files only: ${non_pt}" >&2
        exit 1
    fi
    mapfile -t found_files < <(find -L "${entry}" -mindepth 1 -maxdepth 1 -type f -name '*.pt' -print | sort)
    if [[ ${#found_files[@]} -lt 1 ]]; then
        echo "[psd_analysis.sh] checkpoint directory contains no .pt files: ${entry}" >&2
        exit 1
    fi
    local file
...
if [[ ${#checkpoint_files[@]} -lt 1 ]]; then
    echo '[psd_analysis.sh] no checkpoint files were discovered.' >&2
    exit 1
fi
...
if [[ "${CHECKPOINTS_PER_JOB}" == "all" ]]; then
    ...
    echo '[psd_analysis.sh] CHECKPOINTS_PER_JOB must be a positive integer or all.' >&2
    exit 1
fi
...
for ((start=0; start<${#checkpoint_files[@]}; start+=batch_size)); do
    ...
    log_file="${LOG_DIR}/psd_analysis__${RUN_STAMP}__${slot_id}.log"
    cmd=("${PYTHON_BIN}" -m src.psd_analysis
        ...
        --low_vram "${LOW_VRAM}"
        # Python 인수: --analysis_distance_metric
        # 기능: PSD 곡선 개형거리 계산에 사용할 거리 지표를 전달한다.
        # 줄 수 있는 값: `centered_l2`, `diff_l2`.
        # 값의 의미: `centered_l2` 는 기존 평균 제거 후 L2, `diff_l2` 는 정규화 없는 1차 차분 L2 이다.
        --analysis_distance_metric "${ANALYSIS_DISTANCE_METRIC}"
    ...
    if [[ -n "${SEED}" ]]; then
        ...
        # 줄 수 있는 값: 정수.
        # 값의 의미: analysis 실행 metadata 와 seed 기반 동작에 사용된다.
        cmd+=(--seed "${SEED}")
    fi
    if [[ "${ENABLE_PAIRWISE_DEPENDENCY_APPENDIX}" == "1" || "${ENABLE_PAIRWISE_DEPENDENCY_APPENDIX}" == "true" ]]; then
        # Python 인수: --enable_pairwise_dependency_appendix
        # 기능: pairwise dependency appendix 산출을 켠다.
        # 줄 수 있는 값: 값 없는 flag 이므로, ENABLE_PAIRWISE_DEPENDENCY_APPENDIX 가 참일 때만 붙는다.
        # 값의 의미: 기본 analysis CSV 외 appendix CSV 를 추가한다.
        cmd+=(--enable_pairwise_dependency_appendix)
    fi
    printf -v command_text '%q ' "${cmd[@]}"
    nohup "${cmd[@]}" > "${log_file}" 2>&1
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "${RUN_STAMP}" "psd_analysis" "${case_id}" "${slot_id}" "$!" "${log_file}" "${command_text}" >> "${PID_FILE}"
    launch_count=$((launch_count + 1))
done

printf '[psd_analysis.sh] launched %d job(s); pid file: %s\n' "${launch_count}" "${PID_FILE}"
