...
def test_model_training_ddp_smoke(tmp_path: Path):
    ...
    cfg = {
        'model_training': {
            ...
            'output_root': str(tmp_path / 'out_ddp'),
            'ddp': True,
            'ddp_world_size': 2,
            'batch_size_is_global': True,
        }
    }
    cfg_path = tmp_path / 'smoke_ddp.json'
    ...
    ckpt = tmp_path / 'ckpt_ddp' / 'checkpoint_epoch_000001.pt'
    metric = tmp_path / 'metric_ddp' / 'training_metrics.csv'
    assert ckpt.exists() and metric.exists()
    assert len(list((tmp_path / 'ckpt_ddp').glob('*.pt'))) == 1
