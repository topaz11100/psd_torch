...
def test_psd_metadata_from_args_and_parser_validation():
    ...
    args = parser.parse_args([
        ...
        '--lambda_psd_rep_1d','0.1','--lambda_psd_pca_1d','0.2','--lambda_psd_pca_mimo','0.3','--psd_reg_variant','raw','--psd_reg_output_family','spike'
    ])
    meta = mt._psd_regularization_metadata_from_args(args)
    assert meta['lambda_psd_rep_1d'] == 0.1
    assert meta['lambda_psd_pca_1d'] == 0.2
    assert meta['lambda_psd_pca_mimo'] == 0.3
...
def test_resume_conflict_psd_regularization_metadata():
    current = {
        ...
        'lambda_psd_pca_1d': 0.0,
        'lambda_psd_pca_mimo': 0.0,
        'psd_reg_variant': 'raw',
