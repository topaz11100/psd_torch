"""Checkpoint-only element-wise complex FFT analysis for MLP probe output maps."""
...
SOURCE_PROGRAM = 'element_fft'
VARIANTS = ('raw', 'centered')
COMPONENTS = ('real', 'imag', 'magnitude', 'phase')


def _load_runtime_dependencies() -> None:
    """Element FFT 실행에 필요한 무거운 의존성을 지연 로드한다."""
    ...
    from src.analysis_matrix_common import (
        ProbeMapKey as _ProbeMapKey,
        checkpoint_output_dir as _checkpoint_output_dir,
        collect_mlp_output_maps as _collect_mlp_output_maps,
        common_base_for_key as _common_base_for_key,
        layer_folder as _layer_folder,
        manifest_row as _manifest_row,
        safe_token as _safe_token,
        validate_mlp_only as _validate_mlp_only,
        value_columns as _value_columns,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Checkpoint-only element-wise complex FFT analysis for MLP probe output maps.')
    ...
    parser.add_argument('--output_root', required=True, help='Root directory for element-wise FFT CSV outputs.')
...
def _variant_maps(maps: torch.Tensor, *, variant: str) -> torch.Tensor:
    ...
    raise ValueError(f'Unsupported element FFT variant: {variant!r}.')


def _element_fft_matrix(maps: torch.Tensor, *, variant: str) -> torch.Tensor:
    prepared = _variant_maps(maps.float(), variant=variant)
    if prepared.ndim != 3:
        raise ValueError(f'Expected output maps with shape (samples, neurons, time), got {tuple(prepared.shape)}.')
    return torch.fft.rfft(prepared, dim=-1).mean(dim=0).contiguous()


def _component_matrix(fft_matrix: torch.Tensor, *, component: str) -> torch.Tensor:
    token = str(component)
    if token == 'real':
        return fft_matrix.real
    if token == 'imag':
        return fft_matrix.imag
    if token == 'magnitude':
        return fft_matrix.abs()
    if token == 'phase':
        return torch.angle(fft_matrix)
    raise ValueError(f'Unsupported element FFT component: {component!r}.')


def _value_unit(component: str) -> str:
    token = str(component)
    if token == 'phase':
        return 'radian'
    if token in {'real', 'imag'}:
        return 'complex_component'
    return 'amplitude'


def _artifact_name(base: Mapping[str, Any], key: ProbeMapKey, *, variant: str, component: str) -> str:
    ...
    return (
        f'element_fft__epoch_{safe_token(base.get("checkpoint_epoch", "epoch"))}'
        ...
        f'__{safe_token(sample_role)}__{safe_token(variant)}__{safe_token(component)}.csv'
    )


def _rows_for_element_fft_matrix(
    *,
    ...
    if matrix.ndim != 2:
        raise ValueError(f'Expected element FFT matrix with shape (neurons, frequencies), got {tuple(matrix.shape)}.')
    ...
    for neuron_index in range(neuron_count):
        ...
        row_kwargs.update(
            category='element_fft',
            variant=variant,
            scale='raw',
            fft_component=component,
            ...
            value_unit=_value_unit(component),
        )
...
def _write_element_fft_artifact(
    *,
    checkpoint_dir: Path,
    ...
    fft_matrix = _element_fft_matrix(maps, variant=variant)
    component_matrix = _component_matrix(fft_matrix, component=component).contiguous()
    value_column_names = value_columns('freq', int(component_matrix.shape[1]))
    base = common_base_for_key(checkpoint_base=checkpoint_base, key=key)
    rows = _rows_for_element_fft_matrix(
        base=base,
    ...
    out_path = checkpoint_dir / 'layers' / layer_folder(layer_name, layer_index) / 'element_fft' / _artifact_name(base, key, variant=variant, component=component)
    write_matrix_csv(out_path, rows, value_column_names=value_column_names)
    manifest_rows.append(manifest_row(base=dict(checkpoint_base), artifact_name='element_fft', path=out_path))
...
def _checkpoint_base(
    *,
    ...
    run_id = f'{payload.get("dataset_token", "dataset")}_{model_spec.canonical_token}_{readout_mode}_element_fft_seed{seed}'
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    args = parse_args_with_config(parser, argv=argv, stage_key='element_fft')
    ...
    for checkpoint_path in tqdm(checkpoint_files, desc='element_fft:checkpoints', leave=False):
        ...
        for key, maps in tqdm(sorted(maps_by_key.items(), key=lambda item: (item[0][1], item[0][0], item[0][4], item[0][6] if item[0][6] is not None else -1, item[0][3])), desc='element_fft:artifacts', leave=False):
            for variant in VARIANTS:
                for component in COMPONENTS:
                    _write_element_fft_artifact(
    ...
    manifest_base = first_manifest_base or {'source_program': SOURCE_PROGRAM, 'dataset': str(args.dataset), 'run_id': 'element_fft'}
