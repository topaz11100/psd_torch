...
def main(argv=None):
    ...
    try:
        ...
        if not records:
            fail = AnalysisFailure(status='no_trace_records', reason='no trace records generated', run_id=args.run_id, split='test', probe_family=args.probe_family)
        ...
        if 'unsupported' in msg: status='unsupported_topology'
