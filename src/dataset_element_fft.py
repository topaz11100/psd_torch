...
SOURCE_PROGRAM = 'dataset_element_fft'


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Prepared dataset row/element-wise FFT analysis.')
    ...
    parser.add_argument('--components', nargs='*', default=['real', 'imag', 'magnitude', 'phase'])
...
def _component(values: Any, component: str) -> Any:
    ...
    if token == 'magnitude':
        return values.abs()
    if token == 'phase':
        return torch.angle(values)
    raise ValueError(f'Unsupported FFT component: {component!r}')


def _value_unit_fft(component: str) -> str:
    token = str(component).strip().lower()
    if token == 'phase':
        return 'radian'
    return 'amplitude'
...
def _accumulate_scope(dataset: Any, args: argparse.Namespace, *, device: Any, dtype: Any) -> tuple[dict[str, Any], int, int, int]:
    ...
    for inputs, _targets in loader:
        ...
        for variant in variants:
            prepared = _variant_maps(maps, variant)
            spectrum = torch.fft.rfft(prepared, dim=-1)
            if accum[variant] is None:
                accum[variant] = spectrum.sum(dim=0).detach().clone()
            else:
                accum[variant] = accum[variant] + spectrum.sum(dim=0).detach()
    if sample_count > 0:
        for variant in variants:
...
def _rows_for_matrix(*, base: Mapping[str, Any], matrix: Any, variant: str, component: str, time_length: int) -> list[dict[str, Any]]:
    ...
    for element_index in range(row_count):
        ...
        for frequency_bin, value in enumerate(values.tolist()):
            item = dict(base)
            item.update(
                category='dataset_element_fft',
                signal_kind='input',
                series='x_probe',
                variant=str(variant),
                scale='raw',
                fft_component=str(component),
                ...
                value=float(value),
                value_unit=_value_unit_fft(component),
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    args = parse_args_with_config(parser, argv=argv, stage_key='dataset_element_fft')
    ...
    common_base = {
        ...
        'run_id': f'{bundle.dataset_name}_dataset_element_fft_seed{seed}',
        'seed': int(seed),
    }
    ...
    for scope in _scopes(args.scopes):
        ...
        if sample_count == 0:
            continue
        for variant, complex_matrix in accum.items():
            if complex_matrix is None:
                continue
            for component in _as_list(args.components, ('real', 'imag', 'magnitude', 'phase')):
                matrix = _component(complex_matrix, component)
                base = dict(common_base)
                base.update(split=split_name, scope=f'{split_name}_full', sample_count=int(sample_count))
                rows = _rows_for_matrix(base=base, matrix=matrix, variant=variant, component=component, time_length=time_length)
                out_path = base_output / 'dataset_element_fft' / f'dataset_element_fft__{_safe_token(split_name)}__{_safe_token(variant)}__{_safe_token(component)}.csv'
                write_common_csv(out_path, rows, extra_columns=['sample_count'])
                manifest_rows.append({'scope': f'{split_name}_full', 'artifact_name': 'dataset_element_fft', 'output_csv_path': str(out_path), 'sample_count': int(sample_count)})
    manifest_path = base_output / 'dataset_element_fft_manifest.yaml'
