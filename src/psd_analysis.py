...
"""CLI entry point for the training-time PSD analysis workflow."""
...
from src.util.cli_common import (
...
from src.util.psd_analysis_driver import run_psd_analysis
from src.util.psd_run_configs import PSDAnalysisRunConfig


def build_parser() -> argparse.ArgumentParser:
    """Construct the command-line parser for ``psd_analysis``."""
    ...
    p.add_argument('--model', nargs='+', type=normalize_model_token, required=True, help='One or more model tokens. Recurrent variants may use suffixes like lif_R, lif_struct_R, dh_snn_R, or dh_snn_R_4.')
    ...
    p.add_argument('--rf_reset_mode', type=str, default='no_reset', choices=['no_reset', 'soft_reset'], help='RF reset policy.')
    p.add_argument('--w_clip_edges', nargs='*', type=float, default=None, help='RF clip/structclip frequency-band edges in cycles/sample.')
    p.add_argument('--alpha_clip_edges', nargs='*', type=float, default=None, help='LIF clip/structclip alpha-band edges in normalized alpha space.')
    p.add_argument('--band_neuron_ends', nargs='*', type=str, default=None, help='Optional group endpoint specification for structure scenarios.')
    p.add_argument('--tear', type=int, default=1, help='Group-count multiplier used when generating structured masks.')
    return p


def config_for_combination(args: argparse.Namespace, *, model: str, readout_mode: str) -> PSDAnalysisRunConfig:
    """Build one normalized run configuration for a single model/readout pair."""
    return PSDAnalysisRunConfig(
...
def main() -> None:
    """Parse CLI arguments and execute every requested model/readout combination."""
    parser = build_parser()
    args = parser.parse_args()

    models = dedupe_preserve_order([str(v) for v in args.model])
    readout_modes = dedupe_preserve_order([normalize_readout_mode(str(v)) for v in args.readout_mode])
    validate_model_specific_args(parser, args=args, models=models)

    for model in models:
        for mode in readout_modes:
            config = config_for_combination(args, model=model, readout_mode=mode)
            run_root = run_psd_analysis(**config.to_kwargs())
            print(run_root)
