"""TS-LIF layer aligned with the released ``TSLIFNode`` equations."""
...
def _ts_lif_sequence_no_trace(
    ...
    for i in range(time_steps):
        ...
        v1_pre = d[0] * v1 + d[1] * current - yy * v2
        v2_pre = d[2] * v2 + d[3] * current - kk * v1_pre
        s_s = surrogate_spike(v2_pre - threshold)
        s_l = surrogate_spike(v1_pre - threshold)
        raw = alpha_s * s_s + alpha_l * s_l
        spike = raw if emit_spike else torch.zeros_like(raw)
        if reset_enabled:
            v1 = v1_pre - s_l * gamma
            v2 = v2_pre - s_s * threshold
        else:
            v1 = v1_pre
            v2 = v2_pre
        spike_seq[:, i, :] = spike
...
def _ts_lif_sequence_with_trace(
    projected: torch.Tensor,
    v1: torch.Tensor,
    v2: torch.Tensor,
    prev: torch.Tensor,
...
class TSLIFLayer(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, *, recurrent: bool = False, v_threshold: float = 1.0, emit_spike: bool = True, reset_enabled: bool = True, gamma: float = 0.5) -> None:
        ...
        self.node = origin.TSLIFNode(v_threshold=self.v_threshold, surrogate_function=_OriginCompatibleSurrogate(), gamma=self.gamma, hard_reset=False)
    ...
    def enable_compiled_forward(self, **compile_kwargs: Any) -> tuple[bool, str]:
        no_trace, no_applied, no_policy = compile_callable(_ts_lif_sequence_no_trace, compile_kwargs=compile_kwargs, label='ts_lif_sequence_no_trace')
        with_trace, trace_applied, trace_policy = compile_callable(_ts_lif_sequence_with_trace, compile_kwargs=compile_kwargs, label='ts_lif_sequence_with_trace')
        if no_applied:
    ...
    def _step_impl(self, current: torch.Tensor, v1: torch.Tensor, v2: torch.Tensor, prev_spike: torch.Tensor, recurrent_weight: torch.Tensor | None, decay: torch.Tensor, kk: torch.Tensor, yy: torch.Tensor, alpha_s: torch.Tensor, alpha_l: torch.Tensor, threshold: torch.Tensor, gamma: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.reset_enabled:
            v1_next = v1_pre - s_l * gamma
            v2_next = v2_pre - s_s * threshold
        else:
            v1_next = v1_pre
            v2_next = v2_pre
        return v1_next, v2_next, spike, current, v2_pre, v2_pre - threshold
    ...
    def _run_sequence(self, projected: torch.Tensor, v1: torch.Tensor, v2: torch.Tensor, prev: torch.Tensor, decay: torch.Tensor, kk: torch.Tensor, yy: torch.Tensor, alpha_s: torch.Tensor, alpha_l: torch.Tensor, threshold: torch.Tensor, gamma: torch.Tensor, *, return_traces: bool, record_raw: bool) -> tuple[torch.Tensor | None, torch.Tensor, torch.Tensor | None]:
        if return_traces:
            ...
            if fn is not None and not bool(self._sequence_compiled_runtime_disabled):
                try:
                    mem_seq, spike_seq, inp_seq = fn(projected, v1, v2, prev, self.recurrent_weight, decay, kk, yy, alpha_s, alpha_l, threshold, gamma, self.emit_spike, self.reset_enabled, record_raw)
                    return mem_seq, spike_seq, inp_seq
                except Exception as exc:
                    disable_compiled_runtime(self, label='sequence', exc=exc)
            mem_seq, spike_seq, inp_seq = _ts_lif_sequence_with_trace(projected, v1, v2, prev, self.recurrent_weight, decay, kk, yy, alpha_s, alpha_l, threshold, gamma, self.emit_spike, self.reset_enabled, record_raw)
            return mem_seq, spike_seq, inp_seq
        fn = self._compiled_sequence_no_trace
        if fn is not None and not bool(self._sequence_compiled_runtime_disabled):
            try:
                return None, fn(projected, v1, v2, prev, self.recurrent_weight, decay, kk, yy, alpha_s, alpha_l, threshold, gamma, self.emit_spike, self.reset_enabled), None
            except Exception as exc:
                disable_compiled_runtime(self, label='sequence', exc=exc)
        return None, _ts_lif_sequence_no_trace(projected, v1, v2, prev, self.recurrent_weight, decay, kk, yy, alpha_s, alpha_l, threshold, gamma, self.emit_spike, self.reset_enabled), None
    ...
    def forward(self, input_sequence: torch.Tensor, *, return_traces: bool = False) -> tuple[torch.Tensor | None, torch.Tensor]:
        ...
        decay = self.node.decay_factor.to(device=device, dtype=dtype)
        kk = self.node.kk.to(device=device, dtype=dtype)
        yy = self.node.yy.to(device=device, dtype=dtype)
        alpha_s = self.node.alpha_s.to(device=device, dtype=dtype)
        alpha_l = self.node.alpha_l.to(device=device, dtype=dtype)
        ...
        mem_seq, spike_seq, inp_seq = self._run_sequence(projected, v1, v2, prev, decay, kk, yy, alpha_s, alpha_l, threshold, gamma, return_traces=return_traces, record_raw=record_raw)
