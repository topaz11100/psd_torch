...
from src.signal.filter_property import branch_structure_vectors, ema_mixture_filter_vectors


def _ema_orthogonality_loss(alpha: torch.Tensor) -> torch.Tensor:
    """Closed-form squared cosine similarity for EMA kernels (variable_dendritic_multiplicity.md).
...
def _hard_branch_mask_from_s(s: torch.Tensor, branch: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    Hard branch count rule (variable_dendritic_multiplicity.md):
      - \tilde{s}=min(D,s)
...
class MyDHSNNDenseLayer(nn.Module):
    """Proposed DH-SNN dense spiking layer (spec/Theory/Proposed/my_DH_SNN_neuron.md).
    ...
    def __init__(
        ...
        # Initialization stabilization (variable_dendritic_multiplicity.md §7): W <- sqrt(D) * W
        with torch.no_grad():
    ...
    def harden_branches(self) -> None:
        ...
        Implements variable_dendritic_multiplicity.md:
    ...
    def effective_threshold(self) -> torch.Tensor:
        """Return the soma firing threshold vector (N,)."""
    ...
    def forward_step(self, x_t: torch.Tensor, record: bool = False):
        ...
        threshold = self.effective_threshold().to(device=x_t.device, dtype=x_t.dtype).unsqueeze(0)
        soma_pre = self.mem * beta + (1.0 - beta) * H
        soma_signal = soma_pre - threshold
        raw_spk = self.spike_fn(soma_signal)
        spk = raw_spk if bool(getattr(self, "emit_spike", True)) else torch.zeros_like(raw_spk)
        self.mem = apply_soma_reset(self, soma_pre, spk, threshold)
        self.spk = spk
        ...
        signals = {
            ...
            "soma_state_post_reset": self.mem,
            "output": spk,
        }
        return spk, signals
    ...
    def forward_sequence(self, x_seq: torch.Tensor, record: bool | Sequence[str] = False):
        ...
        if record is False:
            ...
            record_keys = ("dendrite_input", "dendrite_state", "soma_input", "soma_state", "soma_signal", "soma_state_post_reset", "output")
        else:
    ...
    def forward(self, input_sequence: torch.Tensor, *, return_traces: bool = False) -> tuple[torch.Tensor | None, torch.Tensor]:
        """PSD-project forward contract: return optional membrane trace and spike sequence."""
    ...
    def filter_stats_vectors(self) -> Dict[str, torch.Tensor]:
        """Return per-neuron timing, structure, and filter-property vectors."""

        alpha = self.alpha_branch()
        beta = self.beta_soma()
        mask = self.soft_mask(alpha.dtype).to(alpha.device)
        s_value = self.s().to(device=alpha.device, dtype=alpha.dtype)
        vectors: Dict[str, torch.Tensor] = {
            "alpha": alpha.reshape(-1),
            "branch_pole_radius": alpha.reshape(-1),
            "branch_sample_decay_factor": alpha.reshape(-1),
            "beta": beta.reshape(-1),
            "soma_pole_radius": beta.reshape(-1),
            "soma_sample_decay_factor": beta.reshape(-1),
        }
        vectors.update(soma_contract_stat_vectors(self, dtype=alpha.dtype, device=alpha.device))
        vectors.update(branch_structure_vectors(s=s_value, d_int=self.d_int(), mask=mask))
        vectors.update(ema_mixture_filter_vectors(alpha=alpha, mask=mask, s=s_value))
    ...
    def active_param_count(self) -> int:
        ...
        active_threshold = self.output_dim if bool(getattr(self, "trainable_threshold", False)) else 0
        return int(active_syn + active_tau_n + active_tau_m + active_s + active_threshold)
