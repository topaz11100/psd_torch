...
class MyReverseDHSNNDenseLayer(nn.Module):
    ...
    Summary (spec/Theory/Proposed/my_R_DH_SNN_neuron.md):
    ...
    def __init__(
        ...
        # Initialization stabilization (variable_dendritic_multiplicity.md §7): W_mix <- D * W_mix
        with torch.no_grad():
    ...
    def filter_stats_vectors(self) -> Dict[str, torch.Tensor]:
        """Return per-neuron timing, branch-mixing, structure, and filter-property vectors."""

        alpha = self.alpha_branch()
        beta = self.beta_soma()
        mask = self.soft_mask(alpha.dtype).to(alpha.device)
        s_value = self.s().to(device=alpha.device, dtype=alpha.dtype)
        mix = self.W_mix.to(device=alpha.device, dtype=alpha.dtype)
        positive = ((mix > 0.0).to(alpha.dtype) * mask).sum(dim=1)
        negative = ((mix < 0.0).to(alpha.dtype) * mask).sum(dim=1)
        vectors: Dict[str, torch.Tensor] = {
            ...
            "positive_mix_weight_count": positive,
            "negative_mix_weight_count": negative,
        }
        ...
        vectors.update(ema_mixture_filter_vectors(alpha=alpha, mask=mask, s=s_value, mix_weight=mix))
    ...
    def active_param_count(self) -> int:
        ...
        return int(active_W_in + active_Wmix + active_tau_n + active_tau_m + active_s + active_threshold)
