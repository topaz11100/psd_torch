...
이 문서의 서술 순서는 실제 분석 파이프라인을 따른다. 먼저 split / scope 별 deterministic probe set 을 고정하고, 그 다음 선택된 epoch 의 hidden layer 와 output layer signal family 를 수집한다. 이후 같은 signal 에 대해 시간영역 plot, exact periodogram waveform, periodogram userbin heatmap, exact mean spectrogram, spectrogram userbin heatmap 을 순서대로 만든다. 마지막으로 filter 통계, probe-set accuracy, run-level metadata, 학습 완료 시점의 curve-shape semi-metric 요약을 저장한다.
...
입력 probe set 은 epoch 와 무관한 고정 집합이며, 영구 저장 경로는 `paper/proposed/dataset_psd.md` 가 정의한다. `psd_analysis` 는 같은 deterministic probe set index 를 공유하지만 입력 reference PNG 를 직접 저장하지 않는다. 대신 같은 probe batch 에서 계산한 입력 reference payload 를 내부적으로 유지해 hidden / output layer 변화량 비교에만 사용한다.
...
이제 위에서 정의한 signal 요약, 통계, metadata 를 실제 파일 구조에 매핑한다. 각 bundle 의 `summary.json` 은 raw / centered 두 계열의 공통 metadata 와 scalar summary 를 함께 담는다.
...
### 12.2 selected epoch signal bundle
...
### 12.3 centered pointwise L2 semi-metric
...
저장 디렉터리명은 기존 호환성 때문에 `derivative_semi_metric/` 을 유지하지만, 값 정의 자체는 derivative 기반이 아니라 각 곡선을 개별적으로 centering 한 뒤의 pointwise $L_2$ norm 이다.
...
- `summary.json` 에는 centered-$L_2$ 정의, curve-wise centering rule, tracked metric 이름 목록, 그리고 저장 디렉터리명 호환성 note 를 기록한다.
...
- `training_complete_stats/derivative_semi_metric/` 아래 최종 centered-$L_2$ semi-metric plot / json
